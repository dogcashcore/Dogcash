...
The RPM spec file provided here is for DogCash-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://dogcash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DogCash git repository](https://github.com/DogCashProject/DogCash) but are not part of
...
Currently only `Source10` is used, for the example `dogcash.conf` file.
...
in the dogcash git repository. At present only an SVG version of the DogCash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the dogcash
...
needed when 0.12.1 is released, a proper fix is already in the DogCash git
master branch.
...
idea to build DogCash Core against a static version of that library compiled
...
The `dogcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `dogcash-server` package. At this point, most RPM based
...
The `dogcash-server` package should have SELinux support. How to properly do
...
DogCash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/dogcash/). He has been
packaging DogCash for Fedora at least since 2012.
...
4. I renamed the `dogcash` package that contains the Qt GUI to `dogcash-core` as
that appears to be how the general population refers to it, in contrast to
`dogcash-xt` or `dogcash-classic`. I wanted to make sure the general population
