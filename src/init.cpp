 ...
 * between dogcashd, and dogcash-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    FlushWallets();
#endif
    GenerateDogCashs(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), PRODUCTIONCHAIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/DogCashProject/DogCash>";
    const std::string URL_WEBSITE = "<https://dogcash.org>";
...
/** Sanity checks
 *  Ensure that DogCash is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    RegisterAllCoreRDCHommands(tableRPC);
#ifdef ENABLE_WALLET
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single DogCash process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", PRODUCTIONCHAIN_CONF_FILENAME)).string());
    ...
    GenerateDogCashs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
